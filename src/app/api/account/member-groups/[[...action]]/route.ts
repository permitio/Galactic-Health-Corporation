...
const createMemberGroup = async (userId: string, name: string) => {
    const key = `member_group_${userId}_${name
        .toLowerCase()
        .replace(/ /g, '_')}`;
        ...
        return NextResponse.json(
            {
                message:
                    ...
                    'An error occurred, make sure the group name is unique and stand in the correct format',
            },
            {
...
const assignMemberGroup = async (
    ...
    try {
        ...
        const assignedUser =
            users.find(({ email: userEmail }) => userEmail === email)?.id || '';
        ...
        return NextResponse.json(
            {
                message:
                    response?.data?.message ||
                    'An error occurred, make sure this user exists and is not already assigned to this group',
            },
            {
                status: response?.status || 500,
            },
...
const unassignMemberGroup = async (
    userId: string,
    email: string,
    group: string,
) => {
...
const GET = async (request: NextRequest) => {
    ...
    for (const group of userGroups.filter(
        (group) => group.resource_instance?.indexOf('member_group') === 0,
    )) {
        const { key, attributes } = (await permit.api.resourceInstances.get(
            group.resource_instance_id || '',
        )) as MemberGroupInstance;
        ...
        const users = await Promise.all(
            members
                ...
                .map(({ user_id }) => permit.api.users.get(user_id || '')),
        );
